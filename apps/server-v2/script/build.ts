...
function readArgs(argv: string[]): BuildOptions {
  const options: BuildOptions = {
    ...
    embedRuntime: false,
    filename: "openworkplus-server-v2",
...
function runtimeAssetCandidates(bundleDir: string, target?: string): RuntimeAssetPaths {
  ...
  const routerCandidates = [
    triple ? join(bundleDir, `openworkplus-opencode-router-${triple}${triple.includes("windows") ? ".exe" : ""}`) : null,
    join(bundleDir, process.platform === "win32" || target?.includes("windows") ? "openworkplus-opencode-router.exe" : "openworkplus-opencode-router"),
  ];
...
function createEmbeddedEntrypoint(assets: RuntimeAssetPaths) {
  const buildDir = mkdtempSync(join(os.tmpdir(), "openworkplus-server-v2-build-"));
