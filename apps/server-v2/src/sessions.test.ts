...
function createTestApp() {
  const dependencies = createAppDependencies({
    ...
    inMemory: true,
    legacy: {
      desktopDataDir: `/tmp/openworkplus-server-v2-phase6-desktop-${Math.random().toString(16).slice(2)}`,
      orchestratorDataDir: `/tmp/openworkplus-server-v2-phase6-orchestrator-${Math.random().toString(16).slice(2)}`,
    },
    runtime: {
