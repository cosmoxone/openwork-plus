...
export function createRuntimeService(options: CreateRuntimeServiceOptions): RuntimeService {
  ...
  const materializeRouterConfig = () => {
    ...
    const configPath = path.join(dataDir, "openworkplus-opencode-router.json");
    const dbPath = path.join(dataDir, "openworkplus-opencode-router.db");
    const logFile = path.join(dataDir, "logs", "openworkplus-opencode-router.log");
