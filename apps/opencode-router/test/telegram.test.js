...
test("createTelegramAdapter sends text/images/audio/files", async () => {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "openworkplus-opencode-router-tg-send-"));
...
test("createTelegramAdapter downloads inbound media to store", async () => {
  const logger = createLoggerStub();
  const inbound = [];
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "openworkplus-opencode-router-tg-inbound-"));
