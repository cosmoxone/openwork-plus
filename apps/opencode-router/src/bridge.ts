...
const OPENCODE_ROUTER_AGENT_FILE_RELATIVE_PATH = ".opencode/agents/openworkplus-opencode-router.md";
...
export async function startBridge(config: Config, logger: Logger, reporter?: BridgeReporter, deps: BridgeDeps = {}) {
  ...
  const mediaStore = new MediaStore(join(workspaceRoot, ".openworkplus-opencode-router", "media"));
  ...
  const loadMessagingAgentConfig = async (): Promise<MessagingAgentConfig> => {
    ...
    try {
      ...
      logger.warn({ error, filePath }, "failed to load openworkplus-opencode-router agent file");
      latestAgentConfig = { filePath, loaded: false, instructions: "" };
      return latestAgentConfig;
    }
  };
  ...
  async function createSession(input: {
    ...
    const title = `openworkplus-opencode-router ${input.channel}/${input.identityId} ${input.peerId}`;
