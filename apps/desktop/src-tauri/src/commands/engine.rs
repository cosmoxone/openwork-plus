...
pub fn engine_info(
    ...
    if state.runtime == EngineRuntime::Orchestrator {
        ...
        // auth snapshot next to openworkplus-orchestrator-state.json so the UI can reconnect.
...
pub fn engine_start(
    ...
    if runtime == EngineRuntime::Orchestrator {
        ...
        // openworkplus-orchestrator doesn't start its daemon HTTP server until it has ensured that
