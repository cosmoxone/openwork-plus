...
* Runs standalone via `openworkplus-server` CLI.
* Provides filesystem-backed config surfaces (skills, plugins, MCP, commands).
* Sidecar lifecycle is described in `packages/app/pr/openworkplus-server.md`.
...
* Runs standalone via `openworkplus-opencode-router` CLI.
...
* `PRINCIPLES.md`
* `ARCHITECTURE.md`
* `packages/app/pr/openworkplus-server.md`
