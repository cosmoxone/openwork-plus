...
function getLogsDirectory(): string {
  ...
  if (process.env.OPENWORK_DATA_DIR) {
    appDataDir = process.env.OPENWORK_DATA_DIR;
  } else if (platform === 'darwin') {
    appDataDir = path.join(os.homedir(), 'Library', 'Application Support', 'openwork');
  } else if (platform === 'win32') {
    appDataDir = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'openwork');
  } else {
    appDataDir = path.join(os.homedir(), '.config', 'openwork');
  }
