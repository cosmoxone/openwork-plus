...
- Called from and when: called when the `openworkplus-server` binary or `bun src/cli.ts` is launched.
...
- Called from and when: called by the main dispatcher for `/openworkplus-opencode-router` paths.
...
- Called from and when: called for `/openworkplus-opencode-router` and mounted equivalents.
...
- What they do: persist messaging identity config into `openworkplus-opencode-router.json`.
...
- What they are: `/workspace/:id/openworkplus-opencode-router/*` routes.
