...
- Reusable client package: `packages/openworkplus-server-sdk`
...
- `packages/openworkplus-server-sdk/src/streams/**`
...
- `packages/openworkplus-server-sdk`: generated Server V2 client plus small handwritten SSE helpers for Server V2
...
### `packages/openworkplus-server-sdk/package.json`
...
```json
{
  "scripts": {
    ...
    "sdk:generate": "pnpm --filter openworkplus-server-v2 openapi:generate && pnpm --filter @openworkplus/server-sdk generate"
  }
}
```
...
- `packages/openworkplus-server-sdk/scripts/watch.mjs`
...
```bash
pnpm --filter openworkplus-server-v2 openapi:generate && pnpm --filter @openworkplus/server-sdk generate && git diff --exit-code
```
...
- SSE helpers: handwritten in `packages/openworkplus-server-sdk/src/streams/**`
