...
- Ends up calling: `openworkplus-orchestrator start --detach`, token generation, health polling, optional owner-token issuance, and progress event emission.
...
- What it does: launches the `openworkplus-server` sidecar.
...
- What it does: launches `openworkplus-opencode-router serve` with the active OpenCode connection.
