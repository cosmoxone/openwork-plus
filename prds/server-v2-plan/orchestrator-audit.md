...
- starting and supervising `openworkplus-server`
- optionally starting `openworkplus-opencode-router`
...
- What it does: launches `openworkplus-server` with the bootstrap information needed for host mode today.
...
- What it does: launches `openworkplus-opencode-router` when messaging/router support is enabled.
- Called from and when: called from `runStart()` and restart paths.
...
Reasoning: in the current orchestrator these are host/bootstrap concerns, but the Server V2 target changes the default distribution model. When the canonical runtime is `openworkplus-server-v2` bundling and extracting its own sidecars, the equivalent resolution logic should move into the server for that path. A thinner host shell may still need fallback or external-runtime resolution in some modes, so this boundary is better treated as `Split` than `Stay`.

### `resolveOpenworkServerBin()`

- What it does: decides which `openworkplus-server` binary to run.
...
- What they do: read and write `openworkplus-orchestrator-state.json` and related state snapshots.
...
Reasoning: these are not orchestrator responsibilities. They are convenience clients over `openworkplus-server` APIs.
