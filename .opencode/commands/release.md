...
7. If releasing openworkplus-orchestrator sidecars, build deterministically with `SOURCE_DATE_EPOCH`, upload assets to `openworkplus-orchestrator-vX.Y.Z`, and publish `openworkplus-orchestrator`.
8. If `openworkplus-server` or `openworkplus-opencode-router` versions changed, publish those packages.
