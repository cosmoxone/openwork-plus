...
- `/apps/orchestrator/`: OpenWork orchestrator CLI/daemon. In `start`/`serve` host mode it manages OpenWork server + OpenCode + `openwork-plus-opencode-router`; in daemon mode it manages workspace activation and OpenCode lifecycle for desktop runtime.
- `/apps/openwork-plus-opencode-router/`: first-party messaging bridge (Slack/Telegram) and directory router.
...
- `openwork-plus-orchestrator` is the CLI host path for this mode.
...
- Runtime selection in desktop:
  - `openwork-plus-orchestrator` (default): Tauri launches `openwork daemon run` and uses it for workspace activation plus OpenCode lifecycle.
  - `direct`: Tauri starts OpenCode directly.
- In both desktop runtimes, OpenWork server (`/apps/server/`) is the API surface consumed by the UI; it is started with the resolved OpenCode base URL and proxies OpenCode and `openwork-plus-opencode-router` routes.
...
- `openwork-plus-opencode-router` is optional in desktop host mode and is started as a local service when messaging routes are enabled.
...
/apps/desktop (Tauri shell)
    ...
    |          |
    |          +--> OpenCode
    |          +--> /apps/openwork-plus-opencode-router (optional)
    |
    +--> /apps/openwork-plus-opencode-router (optional local child)
```
...
OpenWork can expose a workspace through Slack and Telegram via `openwork-plus-opencode-router`.

- `openwork-plus-opencode-router` is a local bridge that receives messages from messaging adapters and forwards them into OpenCode sessions.
...
OpenWork server proxies `/openwork-plus-opencode-router/*` and `/w/:id/openwork-plus-opencode-router/*` to that local router API.
...
| OpenWork server           |
| /workspace/:id/...        |
| /w/:id/openwork-plus-opencode-router/*  |
+---------------------------+
          |
          ...
          v
+---------------------------+
| local openwork-plus-opencode-router     |
| HTTP control plane        |
+---------------------------+
          |
          v
...
```text
openwork-plus-opencode-router serve /root/workspaces
