...
- `/apps/orchestrator/`: OpenWork orchestrator CLI/daemon. In `start`/`serve` host mode it manages OpenWork server + OpenCode + `openworkplus-opencode-router`; in daemon mode it manages workspace activation and OpenCode lifecycle for desktop runtime.
- `/apps/openworkplus-opencode-router/`: first-party messaging bridge (Slack/Telegram) and directory router.
...
- `openworkplus-orchestrator` is the CLI host path for this mode.
...
- Runtime selection in desktop:
  - `openworkplus-orchestrator` (default): Tauri launches `openwork daemon run` and uses it for workspace activation plus OpenCode lifecycle.
  - `direct`: Tauri starts OpenCode directly.
- In both desktop runtimes, OpenWork server (`/apps/server/`) is the API surface consumed by the UI; it is started with the resolved OpenCode base URL and proxies OpenCode and `openworkplus-opencode-router` routes.
...
- `openworkplus-opencode-router` is optional in desktop host mode and is started as a local service when messaging routes are enabled.
...
/apps/desktop (Tauri shell)
    ...
    |          |
    |          +--> OpenCode
    |          +--> /apps/openworkplus-opencode-router (optional)
    |
    +--> /apps/openworkplus-opencode-router (optional local child)
```
...
OpenWork can expose a workspace through Slack and Telegram via `openworkplus-opencode-router`.

- `openworkplus-opencode-router` is a local bridge that receives messages from messaging adapters and forwards them into OpenCode sessions.
...
OpenWork server proxies `/openworkplus-opencode-router/*` and `/w/:id/openworkplus-opencode-router/*` to that local router API.
...
| OpenWork server           |
| /workspace/:id/...        |
| /w/:id/openworkplus-opencode-router/*  |
+---------------------------+
          |
          ...
          v
+---------------------------+
| local openworkplus-opencode-router     |
| HTTP control plane        |
+---------------------------+
          |
          v
...
```text
openworkplus-opencode-router serve /root/workspaces
